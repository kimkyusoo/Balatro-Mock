...
- 점수 계산하기를 선택하는 경우(Play Hand) 다음의 순서로 진행된다.
  1. 선택된 카드를 기준으로 족보 판정
  2. 판정된 족보의 기본 Chip, Mult 셋팅
  3. 족보에 기여한 카드에 부여된 Chip 순차적 합산
  4. 조커 슬롯에 배치된 조커 카드를 순차적으로 효과 적용하여 Chip, Mult에 이점 부여
  5. 최종 계산된 점수가 Round Score에 합산
  6. Round Score가 상단 Scrore at least에 표시된 목표 점수에 도달하면 승리 및 코인 획득
  7. Hands를 모두 소모하였음에도 도달하지 목표 점수에 도달하지 못하면 GameOver
  8. 승리시 코인 획득 후, 상점으로 이동하여 카드 구매 가능 (조커 카드, 행성 카드, 바우처 카드)
---
### 2.1 기술 설명 - 주요 기술 스택
---
## 2-2. 기술 설명 - 씬 흐름
